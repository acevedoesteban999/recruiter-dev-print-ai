    ...
    def scrape_top_stories(self, pages: int = 5):
        ...
        try:
            for page in range(1, pages + 1):
                url = f"{self.BASE_NEWS_URL}?p={page}" if page > 1 else self.BASE_NEWS_URL
                ...
                for row in rows:
                    try:
                        ...
                        score_elem = row.find_element(By.XPATH, "./following-sibling::tr")
                        score = score_elem.find_element(By.CLASS_NAME, "score").text.split()[0] if score_elem.find_elements(By.CLASS_NAME, "score") else "0"
